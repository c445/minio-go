...
func (c Client) executeMethod(ctx context.Context, method string, metadata requestMetadata) (res *http.Response, err error) {
	...
	for attempt := range c.newRetryTimer(retryCtx, reqRetry, DefaultRetryUnit, DefaultRetryCap, MaxJitter) {
		...
		log.Printf("[minio executeMethod retry]: attempt: %d, reqRetry: %d, isRetryable: %t, object: %s, method: %s, error_code: %s, status_code: %d", attempt, reqRetry, isRetryable, metadata.objectName, req.Method, errResponse.Code, res.StatusCode)
